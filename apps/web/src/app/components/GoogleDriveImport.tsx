...
export function GoogleDriveImport() {
    ...
    tokenClientRef.current = window.google.accounts.oauth2.initTokenClient({
      ...
      callback: (response) => {
        if (response.access_token) {
          ...
          localStorage.setItem("morphic-gdrive-token", JSON.stringify({
  ...
  const fetchFiles = useCallback((token: string, count: number, shared: boolean) => {
  ...
  const downloadOne = useCallback(
    async (file: DriveFile, dirHandle: FileSystemDirectoryHandle | null) => {
      ...
      try {
        ...
        const isWorkspace = file.mimeType.startsWith("application/vnd.google-apps.");

        if (isWorkspace && !exportTarget) {
          throw new Error("This Google Workspace file type cannot be exported.");
        }
        ...
        const downloadName = exportTarget ? file.name + exportTarget.ext : file.name;
  ...
  const allDone = !!files && files.length > 0 && files.every((f) => downloads[f.id]?.status === "done");
        ...
        <div className="rounded-xl border border-neutral-800 bg-neutral-900/40 overflow-hidden">
          <div className="flex items-center justify-between border-b border-neutral-800 px-4 py-3 gap-4">
              ...
              {accessToken && (
                <div className="flex items-center gap-3">
                  <input
                    ...
                    onMouseUp={(e) => applyLimit(Number((e.target as HTMLInputElement).value))}
                    onTouchEnd={(e) => applyLimit(Number((e.target as HTMLInputElement).value))}
                  ...
                  <span className="text-xs text-neutral-500 tabular-nums w-6">{limit}</span>
                </div>
              )}
            </div>
            <div className="flex items-center gap-3 shrink-0">
              <button
                onClick={downloadAll}
                disabled={allDone || files.some((f) => downloads[f.id]?.status === "downloading")}
          ...
          <ul className="divide-y divide-neutral-800">
            {files.map((file) => {
              ...
              const isWorkspace = file.mimeType.startsWith("application/vnd.google-apps.");
              const likelyExportFail = isWorkspace && Number(file.size ?? 0) > 10 * 1024 * 1024;
              return (
                <li key={file.id} className="drive-file-row flex items-center gap-3 px-4 py-3">
                  ...
                  {!likelyExportFail && (
                    <button
                      ...
                      disabled={dl?.status === "downloading" || ds === "deleting" || ds === "deleted"}
                  ...
                  {ds === "deleted" ? (
                    <span className="shrink-0 text-xs text-neutral-500">Deleted</span>
                  ) : ds === "deleting" ? (
                    <span className="shrink-0 text-xs text-neutral-400">Deleting…</span>
                  ) : ds === "confirming" ? (
                    ...
                    <button
                      onClick={() => setDeleteStates((prev) => ({ ...prev, [file.id]: "confirming" }))}
