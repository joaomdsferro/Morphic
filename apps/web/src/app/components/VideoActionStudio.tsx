...
export function VideoActionStudio({ mode }: { mode: VideoMode }) {
  ...
  const actionLabel = useMemo(() => (mode === "convert" ? "Convert videos" : "Compress videos"), [mode]);
  ...
  return (
    <div className="space-y-4">
      <label className="block cursor-pointer rounded-xl border-2 border-dashed border-neutral-700 bg-neutral-900/50 p-6 text-center hover:border-neutral-600 hover:bg-neutral-900">
        ...
        />
        <p className="text-sm text-neutral-300">Drop videos or click to browse</p>
      ...
      {files.length > 0 && (
        <section className="rounded-xl border border-neutral-800 bg-neutral-900/30 p-4">
          <p className="mb-3 text-xs text-neutral-400">{files.length} video file(s) queued</p>
          <ul className="space-y-2">
            {files.map((item) => (
              <li key={item.id} className="rounded-lg border border-neutral-800 bg-neutral-900 px-3 py-2">
                <div className="flex items-center justify-between gap-3">
                  <p className="truncate text-sm text-neutral-200">{item.file.name}</p>
